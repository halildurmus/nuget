  ...
  group('NuGetClient', () {
    ...
    group('autocompletePackageIds', () {
      test('includes pre-release packages in results', () async {
        final response = await client.autocompletePackageIds(
          'win32',
      ...
      test('`skip` parameter must be greater or equal to 0', () async {
        await check(
          client.autocompletePackageIds(null, skip: -1),
        ).throws<RangeError>(
      ...
      test('`take` parameter must be greater than 0', () async {
        await check(
          client.autocompletePackageIds(null, take: 0),
        ).throws<RangeError>(
          (it) => it
    ...
    group('downloadPackageManifest', () {
      ...
      test(
        'downloads a package manifest (with a pre-release version)',
        () async {
      ...
      test(
        ...
        () async {
          await check(
            client.downloadPackageManifest(
              'Non.Existent.Package',
              version: '1.0.0',
            ),
          ).throws<PackageNotFoundException>(
    ...
    group('getAllPackageMetadata', () {
      test('returns metadata for all versions of a package (1)', () async {
        final metadata = await client.getAllPackageMetadata(
          'Microsoft.Win32.Registry',
        );
        check(metadata).isNotEmpty();
        check(metadata.map((m) => m.version)).which(
          (it) => it
      ...
      test(
        ...
        () async {
          await check(
            client.getAllPackageMetadata('Non.Existent.Package'),
          ).throws<PackageNotFoundException>(
            (it) => it
              ..has(
                (e) => e.message,
    ...
    group('getLatestPackageVersion', () {
      ...
      test(
        ...
        () async {
          await check(
            client.getLatestPackageVersion('Non.Existent.Package'),
          ).throws<PackageNotFoundException>(
            (it) => it
              ..has(
                (e) => e.message,
    ...
    group('getPackageMetadata', () {
      test('returns metadata for a package', () async {
        ...
        check(metadata)
          ...
          ..has(
            ...
            'dependencyGroups',
          ).isNotNull().which((it) => it..length.equals(13))
          ...
          ..has(
            (e) => e.title,
            'title',
          ).isNotNull().equals('Microsoft.Win32.Registry')
      ...
      test(
        ...
        () async {
          await check(
            client.getPackageMetadata('Non.Existent.Package', version: '1.0.0'),
          ).throws<PackageNotFoundException>(
            (it) => it
              ..has(
                (e) => e.message,
          ...
          await check(
            ...
            (it) => it
              ..has(
                (e) => e.message,
                'message',
              ).equals('Package `Microsoft.Win32.Registry` (0.1.0) not found.'),
          );
        },
      );
    });
    ...
    group('getPackageVersions', () {
      test(
        'returns all versions of a package (including pre-release versions)',
        () async {
          final versions = await client.getPackageVersions(
            'Microsoft.Windows.SDK.Contracts',
            includePrerelease: true,
          );
          check(versions).which(
            (it) => it
      ...
      test(
        'returns all versions of a package (excluding pre-release versions)',
        () async {
          final versions = await client.getPackageVersions(
            'Microsoft.Windows.SDK.Contracts',
          );
    ...
    group('packageExists', () {
      test('returns true if the package exists', () async {
        await check(
          client.packageExists('Microsoft.Win32.Registry'),
        ).completes((it) => it..isTrue());
      });

      test(
        'returns true if the package with the specified version exists',
        () async {
          await check(
            client.packageExists('Microsoft.Win32.Registry', version: '5.0.0'),
          ).completes((it) => it..isTrue());
        },
      );
      ...
      test('returns false if the package does not exist', () async {
        await check(
          client.packageExists('Non.Existent.Package'),
      ...
      test(
        ...
        () async {
          await check(
            client.packageExists('Microsoft.Win32.Registry', version: '0.1.0'),
          ).completes((it) => it..isFalse());
        },
      );
    });
    ...
    group('searchPackages', () {
      test('includes pre-release packages in results', () async {
        final response = await client.searchPackages(
          'win32',
          includePrerelease: true,
        );
        check(response.totalHits).isGreaterOrEqual(250);
        check(response.data).isNotEmpty();
        check(response.data.map((pkg) => pkg.packageId)).which(
          (it) => it
            ..length.equals(20)
            ..contains('Win32')
            ..contains('Microsoft.Win32.Registry')
        ...
        () async {
          ...
          check(response.data.map((pkg) => pkg.packageId)).which(
            (it) => it
              ...
              ..not(
                (it) => it..contains('Microsoft.Windows.SDK.Win32Metadata'),
              ),
          );
        },
      );
